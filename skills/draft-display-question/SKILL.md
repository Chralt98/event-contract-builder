...
4. Draft the concrete questions according to the reference. For every new event, create at least three distinct selectable units: one direct interpretation and at least two close reformulations, measurable proxies, or better-specified interpretations that preserve the user's intent. A scalar or categorical group counts as one selectable unit even when it contains several questions. Add a template as an additional unit only when every explicit substitution preserves one qualifying predicate, interpretation, settlement source, formula, procedure, methodology, and legal/compliance analysis. A template never replaces one of the three substantive units, and a scalar or categorical unit does not require an unsafe template. A lone binary question may be complete as a domain unit, but it is not enough for a new-event draft. Set a canonical BCP 47 `language_code` from the first request that starts the specification, unless the user explicitly chooses a language. Before calling `submit_drafted_questions`, verify that the draft contains at least three distinct units and that every template has finite, narrow values satisfying the settlement-invariance rules in the reference; if any check fails, revise the draft and do not submit it. Write every question and follow-up in that language. For a new-event draft, call `submit_drafted_questions` exactly once and present its complete returned Markdown with `---` between units and before the follow-up, inserting missing separators as formatting only. Translate renderer-generated labels and other fixed UI text into the locked specification language while preserving every question, variable name/value, and follow-up; do not summarize it. Preserve the returned `forecast_specification_id` and `language_code` so the selected-unit submission and approval can happen before `define-terms`. The immediate next stage after selection is `define-terms`. If the user switches languages while a record is active, remind them of the locked language and offer to continue or start a separate specification in the new language. For a chosen language branch, translate only already-approved stage outputs and obtain new approval for every stage in order; if a selected-unit stage is approved, start it with `submit_selected_unit`, `start_new_specification: true`, the target `language_code`, and no old forecast ID. If no selected-unit stage is approved yet, create a fresh three-unit draft with `submit_drafted_questions` in the target language and no old ID. For a resolution-source alternative, return only the newly drafted display-question unit to that workflow so it can render it as a selectable `alternative_forecast_specification`; do not treat it as selected and do not include definitions.
5. For an existing-question selection, do not generate or restate questions.
   ...
   number, and the carried `forecast_specification_id` and `language_code` when available. Because the user's
   ...
   `submit_selected_unit`, `start_new_specification: true`, the current
   `language_code`, and no original forecast specification ID. Approve that new
   selected unit and obtain fresh approvals for every later stage.
