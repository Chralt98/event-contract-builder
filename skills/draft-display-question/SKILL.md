...
5. For an existing-question selection, do not generate or restate questions.
   ...
   to `define-timing`. The timing stage must propose all timing fields for user
   review and correction; do not ask the user to supply fields without first
   making a complete, clearly provisional proposal. Do not call
   `submit_defined_terms` before timing has also been explicitly approved.
   After timing approval, hand the exact selected unit and approved timing to
   `define-terms`. When the user selects
   an `alternative_market`, start its separate record with
   `submit_selected_unit`, approve that new selected unit, run `define-timing`,
   and omit the original contract ID.
