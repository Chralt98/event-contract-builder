...
plus the selected unit, proposed timing, definitions, and detailed source
...
- `approve_event_contract` records an explicit user approval for one pending MCP-backed stage; it must be called after the matching chat confirmation and in workflow order: selected unit, timing, definitions, then detailed sources.
