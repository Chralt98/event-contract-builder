...
3. After the user agrees to the definitions, use `define-resolution-source` to propose a ranked source hierarchy. First call `propose_resolution_sources` with names, publishers, and exact URLs; after approval, call `submit_resolution_source` with the full source records and present its returned Markdown verbatim.
...
- `propose_resolution_sources` validates and renders a concise ranked source proposal with clickable URLs.
