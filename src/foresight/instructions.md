...
When `approve_forecast_specification` succeeds with `stage: "resolution_criteria"`, the workflow is complete. Present the complete rendered Markdown returned by that approval call, including the selected unit, approved definitions, resolution sources, and resolution criteria. Do not summarize, truncate, or respond with only the approval status or forecast specification ID. If the approval response does not contain the complete rendered result, immediately call `get_approved_forecast_specification` with the returned `forecast_specification_id` and present that tool's complete rendered Markdown instead.
...
- `approve_forecast_specification` records an explicit user approval for one pending MCP-backed stage; it must be called after the matching chat confirmation and in workflow order: selected unit, definitions, detailed sources, then resolution criteria. On final approval, its result must include and render the complete approved forecast specification.
