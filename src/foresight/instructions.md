...
candidate-question draft, workflow follow-ups, or unselected alternatives. After background approval, ask whether the user wants the optional news timeline before offering to show the complete specification. Once the user declines news, no qualifying items are found, or they approve the news timeline, offer to show the complete specification in chat. Call `get_approved_forecast_specification` only when the user chooses to see it.
...
When `approve_forecast_specification` succeeds with `stage: "background_information"`, present only the returned `forecast_specification_id` and exact forecast question text, with `---` between them, then ask whether the user wants the optional news timeline. If they decline or no items qualify after they opted in, offer to show the complete approved specification. If they opt in and approve a news timeline, `stage: "news_timeline"` approval returns only the ID and exact question; then offer to show the complete specification. If the user chooses to see it, call `get_approved_forecast_specification` with that explicit ID and present the complete returned Markdown with the shared separators above, including all approved content and any approved news items. Then ask whether it looks correct; if not, ask what should be changed. Route requested changes through the affected workflow stage, obtain the required approvals again in order, retrieve the updated complete specification, and repeat the review. Do not call `get_approved_forecast_specification` unless the user chooses to see the complete specification.
...
- `approve_forecast_specification` records an explicit user approval for one pending MCP-backed stage; it must be called after the matching chat confirmation and in workflow order: selected unit, definitions, detailed sources, resolution criteria, background information, then optional news timeline. Ask whether to enter the news stage after background approval; never proceed automatically. Final approval returns the language code in structured content and shows only the forecast specification ID and exact question text in its user-facing summary.
